import { map, mapObjIndexed, prop, repeat } from 'ramda'
...
export default (targets = {}, {
  ...
  const initialValue = {healthy: true, cached: false, error: null}

  const checks = map(createHealthCheck)(targets)
  const events = map(createHealthEvents)(checks)
  const sources = map(e => createHealthObservables(e, {
    initialValues: repeat(initialValue)(graceChecks)
  }))(events)
