...
test('creates observables with initial value', async t => {
  ...
  const events = createHealthEvents(healthCheck)
  const observables = createHealthObservables(events, {
    initialValues: [{healthy: true, cached: false, error: null}]
  })
  keys.forEach(k => observables[k].subscribe(x => { t.true(x !== undefined) }))
})
...
test('creates uptime observable', async t => {
  ...
  t.true(value() >= 2 * dt && value() < 3 * dt, 'after 3 checks')
  ...
  await t.context.emit(false)
  t.true(value() >= 4 * dt && value() < 5 * dt, 'after 5 checks')
})
...
test('creates downtime observable', async t => {
  ...
  await sleep(1 * dt) // down
  await t.context.emit(true)
  t.true(value() >= dt && value() < 2 * dt, 'after 4 checks')

  await sleep(dt) // up
  await t.context.emit(false)
  await sleep(3 * dt) // down
  await t.context.emit(false)
  t.true(value() >= 4 * dt && value() < 5 * dt, 'after 6 checks')
})
