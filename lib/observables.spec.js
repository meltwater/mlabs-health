...
test('creates duration observable', async t => {
  ...
  await sleep(dt)
  await t.context.emit(true)
  t.true(value() >= dt && value() < 2 * dt, 'after 2 check')

  await sleep(3 * dt)
  await t.context.emit(true)
  t.true(value() >= 4 * dt && value() < 5 * dt, 'after 3 check')
  ...
  await t.context.emit(true)
  await t.context.emit(true)
  t.true(value() >= 3 * dt && value() < 4 * dt, 'after 6 check')
})
