...
test('creates duration observable', async t => {
  const dt = 220
  const source = duration(t.context.source).startWith(0)
  const value = createValue(source)
  t.is(value(), 0, 'for initial value')
  ...
  await sleep(dt)
  await t.context.emit(true)
  t.true(value() >= dt && value() < 2 * dt, 'after 2 checks')

  await sleep(3 * dt)
  await t.context.emit(true)
  t.true(value() >= 4 * dt && value() < 5 * dt, 'after 3 checks')
  ...
  await t.context.emit(true)
  await t.context.emit(true)
  t.true(value() >= 3 * dt && value() < 4 * dt, 'after 6 checks')
})
...
test('creates uptime observable', async t => {
  const dt = 220
  const source = uptime(t.context.source).startWith(0)
  const value = createValue(source)
  t.is(value(), 0, 'for initial value')
  ...
  await sleep(dt) // up
  await t.context.emit(true)
  t.true(value() >= dt - 3 && value() < 2 * dt, 'after 2 checks')

  await sleep(dt) // up
  await t.context.emit(false)
  await sleep(1 * dt) // down
  t.true(value() >= 2 * dt - 3 && value() < 3 * dt, 'after 3 checks')
  ...
  await t.context.emit(false)
  t.true(value() >= 4 * dt - 3 && value() < 5 * dt, 'after 5 checks')
})
...
test('creates downtime observable', async t => {
  const dt = 220
  const source = downtime(t.context.source).startWith(0)
  const value = createValue(source)
  t.is(value(), 0, 'for initial value')
  ...
  await sleep(1 * dt) // down
  await t.context.emit(true)
  t.true(value() >= dt - 3 && value() < 2 * dt, 'after 4 checks')

  await sleep(dt) // up
  await t.context.emit(false)
  await sleep(3 * dt) // down
  await t.context.emit(false)
  t.true(value() >= 4 * dt - 3 && value() < 5 * dt, 'after 6 checks')
})
...
test('creates availability observable', async t => {
  const dt = 220
  const source = availability(t.context.source).startWith(1)
  const value = createValue(source)
...
test('creates reliability observable', async t => {
  const source = reliability(t.context.source).startWith(1)
  const value = createValue(source)
  t.is(value(), 1, 'for initial value')
