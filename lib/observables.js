...
import { BehaviorSubject, fromEvent, pipe as rxPipe, zip as rxZip } from 'rxjs'
import {
  filter as rxFilter,
  map as rxMap,
  pluck as rxPluck,
...
export const duration = source =>
  rxZip(healthy(source), date(source)).pipe(
    rxPipe(
      rxScan(
...
export const count = source =>
  rxZip(healthy(source), healthy(source).pipe(rxMap(() => 1))).pipe(
    rxPipe(
      rxScan(
...
export const availability = source =>
  rxZip(uptime(source), downtime(source), (up, down) =>
    up === 0 && down === 0 ? 1 : up / (up + down)
  )

export const reliability = source =>
  rxZip(attempts(source), successes(source), (attempts, successes) =>
    attempts === 0 ? 1 : successes / attempts
  )
...
export const status = (source, sources = {}) => {
  ...
  return source.pipe(
    rxWithLatestFrom(...map(k => sources[k])(keys), (health, ...stats) =>
      mergeAll(stats.map((stat, i) => ({ [keys[i]]: stat })))
    )
  )
}
...
export default (
  ...
  const source = createSource(events).pipe(
    rxFilter(({ cached }) => (ignoreCached ? !cached : true))
  )

  const delayedSource = rxZip(source, healthy(source), date(source)).pipe(
    rxPipe(
      rxFilter(
        ([x, isHealthy, currentDate]) =>
