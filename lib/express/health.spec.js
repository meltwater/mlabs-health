...
test('fails to create health middleware', async t => {
  t.throws(() => expressHealth(), /missing/i, 'when missing first argument')

  t.throws(
    () => expressHealth({healthy: () => true}),
    /missing 'healthMonitor'/i,
    'when missing health monitor'
  )

  t.throws(
    () => expressHealth({healthMonitor: {}}),
    /missing 'healthy'/i,
    'when missing healthy function'
  )
})
