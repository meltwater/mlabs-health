...
test('creates status middleware with date', async t => {
  const date = new Date()
  const app = express()
  const status = {healthy: false, unhealthy: true, date}
  app.get('/', expressStatus({status: () => status}))
  const res = await request(app).get('/')
  t.is(res.status, 200, 'where status is set')
  t.deepEqual(res.body, {data: {...status, date: date.toString()}}, 'where body is json')
})
...
test('creates status middleware with meta and no data', async t => {
  const app = express()
  const status = {healthy: true, unhealthy: false, error: null}
  app.get('/', expressStatus({status: () => status, meta: () => ({id: 'foo'})}))
  const res = await request(app).get('/').query({data: false})
  t.is(res.status, 200, 'where status is set')
  t.deepEqual(res.body, {data: null, id: 'foo'}, 'where body is json')
})
