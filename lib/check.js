...
const formatHealth = (health, cached) => {
  ...
  return {healthy: null, cached, error: null}
}
...
export default (target, {
  ...
  return async (...args) => {
    try {
      const { healthy, cached, error } = await checkHealth(...args)
      if (healthy === null) return {healthy: true, cached, error: null}
      return {healthy, cached, error}
    } catch (error) {
