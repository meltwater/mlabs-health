...
import { has, isNil, sleeP } from '@meltwater/phi'
...
export default (target, {
  ...
  const checkHealth = async (...args) => {
    ...
    const health = cache === null
      ? await getHealthWithTimeout(...args)
      : await cache.wrap(id, () => {
        cached = false
        return getHealthWithTimeout(...args)
      })
  ...
  return async (...args) => {
    try {
      const health = await checkHealth(...args)
      if (health === null) return {healthy: true, cached, error: null}
      return health
    } catch (error) {
